...
ACTIVE_SESSION_PATH = os.path.join(ROOT, "active_session.txt")
...
def get_alerts(session_id: int):
    return query("""
        ...
        FROM events
        WHERE session_id = ?
          AND (hand_raised = 1 OR sleeping = 1 OR COALESCE(phone_detected, 0) = 1)
        ORDER BY timestamp DESC
        LIMIT 50
    """, (session_id,))
...
def pipeline_status():
    return {"running": os.path.exists(ACTIVE_SESSION_PATH)}
