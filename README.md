...
- 🧠 Calculates focus score using head pose estimation with MediaPipe
- 🙋 Detects hand raises and sleeping via pose estimation
- 📱 Detects phone usage and penalizes focus score
- 🎙️ Transcribes teacher speech every 30 seconds using OpenAI Whisper
...
- ▶️ Allows starting and stopping the pipeline from the dashboard
- 📚 Lets users review past sessions in the Session Analysis tab
...
| Action | What Happens |
|---|---|
| Click "Upload Video" | Switches the pipeline from live webcam to a pre-recorded video file |
| Look at camera | Focus score goes up (green) |
| Look away | Focus score drops (red) |
| Raise hand | HAND RAISED alert appears on the dashboard |
| Hold phone | ON PHONE alert appears and focus is penalized |
| Slump head forward | SLEEPING is detected |
| Speak into mic | Transcript and Q-ratio appear after 30 seconds |
